...
let () =
  ...
  let ast = Clang.TranslationUnit.parse_file_internal [| Sys.argv.(1) |] in
  ...
  let rec insert_comment decl =
    ...
    | Some decl -> (
        ...
        | Clang.DeclKind.FunctionDecl ->
            let _ =
              Clang.Rewriter.insert_before_decl decl "/* inserted */\n" rw
            in
            ()
        | _ ->
  ...
  Clang.Rewriter.emit_string rw |> print_endline;
